...
def surface_measurement(circuit_builder, distance , offset, tag, postselect = False):
    for height in range(distance - 1):
        for width in range(distance - 1):
            if (height + width) % 2 == 0:
                circuit_builder.mrz((1.5 + width + offset[0], 1.5 + height + offset[1]), tag)
...
def surface_data_initialization_expand(circuit_builder, distance, offset):
    ...
    for height in range(distance):
        for width in range(distance):
            if height < distance_0 and width < distance - distance_0:
                circuit_builder.rz((1 + width + offset[0], 1 + height + offset[1]))
            if height >= distance_0 and width >= distance - distance_0:
                circuit_builder.rx((1 + width + offset[0], 1 + height + offset[1]))
            if height >= distance_0 and width < distance - distance_0:
                hei = height - distance_0
                wid = distance - distance_0 - width - 1
                if wid <= hei:
                    circuit_builder.rx((1 + width + offset[0], 1 + height + offset[1]))
                else:
                    circuit_builder.rz((1 + width + offset[0], 1 + height + offset[1]))
...
def surface_measurement_first_expand(circuit_builder, distance, offset, tag):
    cnt = 0
    distance_0 = 3
    decided = [0, 2, 4, 9, 11, 16, 18, 25, 30, 32, 37, 39, 44, 46, 51, 53, 55, 58, 60, 62]
    for height in range(distance - 1):
        for width in range(distance - 1):
            if (height + width) % 2 == 0:
                circuit_builder.mrz((1.5 + width + offset[0], 1.5 + height + offset[1]), tag)
                if cnt in decided:
                    circuit_builder.detector([-1], False, True)
                cnt += 1
                tag += 1
            else:
                circuit_builder.mrx((1.5 + width + offset[0], 1.5 + height + offset[1]), tag)
    ...
    for width in range(2):
        for height in range(distance // 2):
            circuit_builder.mrz((0.5 + width * distance + offset[0], 2.5 + height * 2 - width + offset[1]), tag)
            if width == 0 and height < distance // 2 - 1:
                circuit_builder.detector([-1], False, True)
            tag += 1
    for height in range(2):
        for width in range(distance // 2):
...
def surface_measurement_boundary(circuit_builder, distance, offset, tag, postselect = False):
    height = 0
    for width in range(distance // 2):
        circuit_builder.mrx((1.5 + width * 2 + height + offset[0], 0.5 + height * distance + offset[1]), tag)
...
def surface_measurement_rot90(builder, distance, offset, tag, postselect):
    proxy = RotatedBuilderProxy(builder, _rotation_center(distance, offset))
    return surface_measurement(proxy, distance, offset, tag, postselect)

def surface_measurement_boundary_rot90(builder, distance, offset, tag, postselect):
    proxy = RotatedBuilderProxy(builder, _rotation_center(distance, offset))
    return surface_measurement_boundary(proxy, distance, offset, tag, postselect)
...
def surface_measurement_mirror_ud(builder, distance, offset, tag, postselect):
    proxy = MirrorUpDownBuilderProxy(builder, _symmetry_center(distance, offset))
    return surface_measurement(proxy, distance, offset, tag, postselect)

def surface_measurement_boundary_mirror_ud(builder, distance, offset, tag, postselect):
    proxy = MirrorUpDownBuilderProxy(builder, _symmetry_center(distance, offset))
    return surface_measurement_boundary(proxy, distance, offset, tag, postselect)
